...
import org.smurve.yainn.components.{AutoUpdatingAffine, Layer}
...
object Ex_06_CompareNaiveWithADAMExperiment extends AbstractMNISTExperiment with Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val nn_a = createNetwork_with_Adam(params.SEED, params.ETA_ADAM, 784, 1400, 100, 10)
  ...
  def createNetwork_with_Adam(seed: Long, eta: Double, dims: Int*): Layer =
    (for (((left, right), index) <- dims.zip(dims.tail).zipWithIndex) yield {
      ...
      AutoUpdatingAffine(name, new L2RegAffineParameters(left, right, 0.0, seed,
        ...
        (if (index == dims.size - 2) SoftMaxCrossEntropy() else Relu())
